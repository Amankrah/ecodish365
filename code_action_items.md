...
**Out of scope (follow-ups explicit).** A posteriori PCA / RRR pattern derivation from CCHS Public-Use Microdata (Hu 2002 *Curr Opin Lipidol* 13:3 method; blocked on RDC access); new a priori pattern index ("Pattern-HEFI") that would require its own validation; outcome-prediction modelling at the user level (the platform doesn't collect health-outcome data); persistent per-user pattern history (no auth in v1); substantive swap-recommendation engine beyond the LLM narrative's casual third-sentence suggestion; custom user-defined prototypes; SVG radar chart (v1 uses horizontal cosine bars); multi-day usual-intake pattern modelling (NCI MCMC — requires multiple recalls per user, same Brassard 2022b limitation we documented for the recall wizard).
