...
export function SubstitutionSuggestionsPanel({
  ...
  const [maxSwaps, setMaxSwaps] = useState<1 | 2 | 3 | 4>(1);
  const [reformulationMode, setReformulationMode] = useState<SubstitutionReformulationMode>('singles');
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-gray-600">
          ...
          improve and click <strong>Find swaps</strong>. We suggest replacements that keep the
          same portion size so you can compare scores fairly.
        </p>
      ...
      {!loading && suggestions.length > 0 && (
        <div className="space-y-3">
          {suggestions.map((s, i) => {
            ...
            const onFrontier = s.pareto?.on_frontier ?? frontierIds.has(sid);
            const tag = sourceLabel(s.candidate_source, userType);
            return (
              <div
                key={`${s.rule_id}-${s.ingredient_index}-${i}`}
                className={`border rounded-lg p-4 ${
                  onFrontier ? 'border-amber-300 bg-amber-50/50' : 'border-violet-200 bg-violet-50/40'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => onApply(s.modified_composition, s)}
                    className="flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-md bg-violet-600 text-white hover:bg-violet-700"
                  >
                    Apply
                  </button>
                </div>
