...
import { FpedPanel } from '../shared/FpedPanel';
              ...
              <CollapsibleSection
                ...
                whenCollapsedHint={
                  <p className="px-6 pb-4 text-xs text-gray-600">
                    {userType === 'individual'
                      ? 'Click above to search for foods and add or change your list.'
                ...
                <div className="p-6 pt-0 space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {userType === 'individual'
                        ? 'Search and select foods to include in your health impact score.'
                        : 'Search and select foods to include in the HENI calculation.'}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      🍳 {userType === 'individual' ? 'Break down a homemade dish' : 'Score a homemade dish (decompose into CNF ingredients)'}
                    </button>
                    <a
                      href="/recall-24h?then=heni"
                      className="inline-flex items-center gap-1.5 text-sm text-green-700 hover:text-green-900 hover:underline"
                    >
                      🍽️ {userType === 'individual' ? 'Log a full food diary day instead' : 'Build a 24-h recall instead (six-occasion daily eating)'}
                    </a>
          ...
          <TabsContent value="analysis" className="space-y-6">
            {heniResults && (
              <>
                ...
                {/* FPED-1: food-group exposure paired with the HENI health-minutes
                    read — "your processed-meat / whole-grain / veg equivalents vs a
                    balanced plate", the food-group counterpart to the μDALY story. */}
                <FpedPanel
                  foods={selectedFoods.map(f => ({ food_id: f.FoodID, mass_g: f.amount }))}
                  userType={userType}
                />
