    ...
    <div className="bg-white border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="text-sm text-gray-600 max-w-xl">
          ...
          {userType === 'researcher' && (
            <span>Lens-relevant nutrients, provenance, and all-scores handoff. Values are per 100 g unless noted.</span>
          )}
          {userType === 'policy' && (
            <span>Population-level catalogue browsing. Export comparisons or send foods to all scores for multi-lens review.</span>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-3">
          {activeFoodCount > 0 && (
            ...
            >
              ...
              {userType === 'individual' ? 'All scores' : 'Scorecard'} ({activeFoodCount} food{activeFoodCount === 1 ? '' : 's'})
            </Link>
          )}
