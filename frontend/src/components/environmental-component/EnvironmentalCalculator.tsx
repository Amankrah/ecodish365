...
const EnvironmentalCalculator = () => {
  ...
  // AGRIBALYSE-INGEST §3.5: experimental Agribalyse-3.2 matcher toggle.
  // Default off; opt-in until Scenario S7 (300-pair expert validation) lands.
  const [enableLcaMatcher, setEnableLcaMatcher] = useState<boolean>(false);
  ...
  const calculateEnvironmentalImpact = async () => {
    ...
    try {
      ...
      const response = await EnvironmentalImpactApiService.analyzeMealEnvironmentalImpact({
        foods,
        user_type: userType,
        enable_lca_matcher: enableLcaMatcher,
      });
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <Card className="shadow-lg">
                ...
                <CardContent className="space-y-6">
                  ...
                  {/* AGRIBALYSE-INGEST §3.5 matcher toggle (experimental). */}
                  <div className="mt-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <label className="flex items-start gap-3 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={enableLcaMatcher}
                        onChange={(e) => setEnableLcaMatcher(e.target.checked)}
                        className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
                      />
                      <div className="text-sm">
                        <div className="font-medium text-gray-900">
                          Use Agribalyse 3.2 matching (experimental)
                        </div>
                        <p className="text-xs text-gray-600 mt-0.5">
                          For each food, an LLM retrieves the closest Ciqual entry
                          from the 2,425-row v32 catalog and overlays its factors on
                          the climate-change pathway (other 16 ReCiPe categories
                          fall back to group defaults). Canadian regional scaling
                          is suppressed on matched categories because Agribalyse
                          already encodes FR/EU geography. Adds ~2 s and ~$0.0002
                          per food. Default off pending Scenario S7 validation.
                        </p>
                      </div>
                    </label>
                  </div>
