...
export default function SavedMealsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const resp = await MealApiService.getSavedMeals({ ordering });
        // Normalize handler on service already returns { results }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-gray-600">Loading your saved meals...</p>
            </div>
          ) : meals.length === 0 ? (
            <div className="text-center py-12">
